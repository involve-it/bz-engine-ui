(function ($) {
        ...
        select.find('option')
          .each(function () {
            if ($(this)
              .attr('selected')) {
              selected = 'selected';
              selectPrompt = '<div class=\'' + $(this)
                .attr('class') + '\'>' + $(this)
                .html() + '</div>';
            }
            if ($(this)
              .attr('class')) {
              translateClasses = $(this)
                .attr('class') + ' ';
            }
            options += '<li data-value="' + this.value + '" class="' + translateClasses + selected + '"><span class="option-title">' + $(this)
              .html() + '</span></li>';
            selected = '';
          });
  ...
  $(document)
    .on('click', '.custom-dropdown-area li', function () {
      if ($(this)
        .hasClass('disabled')) {
        return false;
      }
      dropdown = $(this)
        .closest('.custom-dropdown-area');
      ...
      if (multiple) {
        ...
        if (selectedOptions.length) {
          if (selectedOptions.length > 2) {
            dropdown.find('.custom-dropdown-button')
              .html(selectedOptions.length + ' of ' + totalOptions + ' selected');
          } else {
            dropdown.find('.custom-dropdown-button')
              .html(selectedTitles.join(', '));
          }
        } else {
          dropdown.find('.custom-dropdown-button')
            .html(prompt);
        ...
        Foundation.libs.dropdown.close($('#' + dropdown.find('ul')
          .attr('id')));
  ...
  $(document)
    .on('reset', 'form', function () {
      if ($(this)
        .children('.custom-dropdown-area').length) {
        $(this)
          .find('.custom-dropdown-area')
          .each(function () {
            origDropdown = $($(this)
              .data('orig-select'));
            dropdown = $(this);
            ...
            if (origDropdown.data('prompt')) {
              ...
              origDropdown.find('option')
                .each(function () {
                  if ($(this)
                    .attr('selected')) {
                    prompt = $(this)
                      .html();
                    dropdown.find('li[data-value="' + this.value + '"]')
                      .addClass('selected');
                  }
                });
              if (prompt == '') {
